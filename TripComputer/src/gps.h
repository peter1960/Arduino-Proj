...
private:
    ...
    uint8_t m_step = 0; // State machine state
    uint8_t m_msg_id;
    uint16_t m_payload_length;
    uint16_t m_payload_counter;
    uint8_t m_ck_a; // Packet checksum accumulators
    uint8_t m_ck_b;
    uint8_t m_SatCount;
    uint8_t m_Accuracy;
    int32_t m_GPS_coordLON;
    int32_t m_GPS_coordLAT;
    uint32_t m_GPS_speed;
    uint32_t m_avg_GPS_speed[SPEED_SAMPLES];
#ifdef PL_DEBUG_GPS
    uint32_t updateNoFixTime = 0; // time for next update
    uint32_t updateFixTime = 0;   // time for next update
#endif
};
...
const unsigned char UBLOX_INIT[] PROGMEM = {
    ...
    0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x03, 0x00, 0xFD, 0x15,                               // GPS Satellites in View
    0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x01, 0x00, 0xFB, 0x11,                               // Latitude and longitude, with time of position fix and status
    0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x00, 0x00, 0xFA, 0x0F,                               // Global positioning system fix data
    0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x02, 0x00, 0xFC, 0x13,                               // GPS DOP and Active Satellites
    0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x04, 0x00, 0xFE, 0x17,                               // Recommended Minimum data
